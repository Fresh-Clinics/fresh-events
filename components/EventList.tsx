    ...
    <div className="grid gap-4">
      {futureEvents.map(({ event, api_id }) => (
        <a className="event-box relative" key={api_id} href={event.url} target="_blank" rel="noopener noreferrer">
          <div className="flex">
            <div className="jsx-2370077516 icon-container flex-center-center rounded overflow-hidden flex-shrink-0 mr-4">
              <div className="text-center">
                <div className="text-lg font-bold">{moment(event.start_at).format('DD')}</div>
                <div className="text-sm">{moment(event.start_at).format('MMM')}</div>
              </div>
            </div>
            <div className="grid gap-1 pr-[20%] flex-grow">
